...
pull_mom6_hindcast <- function(var_url, req_vars, short_names, gt = 'regrid', of = 'monthly', bounds = c(-78,-65, 35,45), static, release){
  ...
  for(y in 1:length(req_vars)){
    ind <- which(long.name == req_vars[y] & grid.type == gt & out.freq == of  & rl == release) #find appropriate url for the variable
