  ...
  if (model == 'sdmtmb') {
    ...
    for (i in 1:5) {
      ...
      tryMod <- tryCatch(
        ...
        error = function(e) {
          #to do if fails - this essentially will skip the
          print(
            'could not build model for test - be sure to test how many iterations failed. may need to be rerun if 2+ fail.'
          )
