...
make_sdm_predictions <- function(
  ...
  if (model == 'ensemble') {
    ...
    hsm <- vector(mode = 'list', length = terra::nlyr(rasts[[1]]))

    for (x in seq_along(hsm)) {
      abunds <- lapply(rasts, function(r_list) r_list[[x]])
      hsm[[x]] <- EFHSDM::MakeEnsembleAbundance(
  ...
  if (model == 'sdmtmb') {
    warning(
      'sdmTMB predictions can take a long time (approximately 1 min per timestamp).'
    )
  }
  ...
  for (x in 1:num_layers) {
    ...
    if (model == 'gam') {
      ...
      hsm[[x]] <- terra::rast(EFHSDM::MakeGAMAbundance(
        model = mod,
        r.stack = sr_legacy
      ))
      ...
      hsm[[x]] <- EFHSDM::MakeMaxEntAbundance(
        model = mod,
        maxent.stack = sr_legacy,
        type = 'maxnet'
      )
    } else if (model == 'brt') {
      ...
      v_fit_anis <- gstat::fit.variogram(
        v_geo,
        model = gstat::vgm(
          psill = 0.06,
          model = "Sph",
      ...
      kriged_df <- as.data.frame(gstat::krige(
        val ~ 1,
        pts_df,
        grid_pts,
        model = v_fit_anis,
