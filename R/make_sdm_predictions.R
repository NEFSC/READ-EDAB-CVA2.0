  ...
  if (model %in% c('gam', 'maxent', 'rf', 'brt', 'sdmtmb', 'ens')) {
    ...
    if (model == 'maxent') {
      ...
      for (x in 1:raster::nlayers(rasts[[1]][[1]])) {
        ...
        hsm[[x]] <- raster::raster(EFHSDM::MakeMaxEntAbundance(
          model = mod,
          maxent.stack = sr,
          type = 'maxnet'
        ))
    ...
    if (model == 'sdmtmb') {
      ...
      warning(
        'sdmTMB predictions can take a long time in some environments. If this call is taking too long (> 1 hr), try switching to predictSDM and makePredDF'
      )
      for (x in 1:raster::nlayers(rasts[[1]][[1]])) {
        #all the rasters in rasts have the same number of layers so it doesn't matter which one we call
    ...
    if (model == 'ens') {
      ...
      if (length(rasts) == length(weights)) {
        ...
        for (x in 1:length(rasts[[1]])) {
          ...
          hsm[[x]] <- raster::raster(EFHSDM::MakeEnsembleAbundance(
            model.weights = wts,
            abund.list = abunds
          ))
        } #end x
