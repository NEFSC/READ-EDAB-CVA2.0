...
#' @param coastline shapefile used to plot land in stock boundary plots
...
make_stock_polygons <- function(
  ...
  for (x in names) {
    ...
    for (s in stocks) {
      ...
      spp.stock.shp <- polygons[
        polygons[[poly_id]][, 1] %in% spp.stock[, id_col],
      ] #subset geometry

      # --- NEW SAFETY CATCH ---
      if (nrow(spp.stock.shp) == 0) {
        warning(paste(
          "No matching polygons found for stock:",
          s,
          "- skipping."
        ))
        next
