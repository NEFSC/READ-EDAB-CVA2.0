...
build_fisheries_df <- function(data, is_obs, grid, tm_multiplier = 24 * 60 * 60, origin = '1993-01-01', all_names){
  ...
  dataIND <- TRUE #most data come from fisheries independent data sources, so assume independence unless proven otherwise below
  if(is_obs == TRUE){
  ...
  for(x in 1:nrow(my)){ #for each month.year
    ...
    if(nrow(sub) != 0){
      ...
      for(i in ids){
        ...
        iLon <- DescTools::Closest(x = lonR-360, a = tow$lon[1], which = F)
