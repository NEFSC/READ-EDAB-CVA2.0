...
build_fisheries_df <- function(
  ...
  if (is_obs == TRUE) {
    ...
    if (length(which(iSPP == T)) <= 30) {
      #so if these requirements are not met, change flag & print warning
      print(
        'Observer data do NOT meet minimum thresholds to include in data.frame...make sure that all possible variations of the species name (scientific, common, alternative common names) are included in targetVec'
      )
      depCheck <- FALSE
    }
  } #end if observer
