...
apply_sensitivity_logic_rule <- function(attribute_vec, bootstrap) {
  if (bootstrap) {
    #if bootstrap = T, attribute_vec will be a MATRIX; apply the logic rule to each ROW
    total.score <- vector(length = nrow(attribute_vec))
    for (x in 1:nrow(attribute_vec)) {
      if (length(which(attribute_vec[x, ] >= 3.5)) >= 3) {
        # Very High
        total.score[x] <- 4
      } else if (length(which(attribute_vec[x, ] >= 3)) >= 2) {
        # High
        total.score[x] <- 3
      } else if (length(which(attribute_vec[x, ] >= 2.5)) >= 2) {
        # Moderate
        total.score[x] <- 2
      } else {
        # Low
    ...
    #if bootstrap == F, attribute_vec will be a vector; apply logic rule to vector only
    total.score <- NULL
    if (length(which(attribute_vec >= 3.5)) >= 3) {
      # Very High
      total.score <- 4
    } else if (length(which(attribute_vec >= 3)) >= 2) {
      # High
      total.score <- 3
    } else if (length(which(attribute_vec >= 2.5)) >= 2) {
      # Moderate
      total.score <- 2
    } else {
      # Low
