...
attribute.score <- function(attribute, bootstrap = TRUE, samples = 10000){
  ...
  if(bootstrap){ #if bootstrap is TRUE, perform bootstrap
    ...
    score <- (sum(scores.only$Scoring.Rank1, na.rm = T) + sum(scores.only$Scoring.Rank2, na.rm = T)*2 + sum(scores.only$Scoring.Rank3, na.rm = T)*3 + sum(scores.only$Scoring.Rank4, na.rm = T)*4) / sum(scores.only, na.rm = T)
