...
#' @param plots.file.name desired name of plot file. file extension not needed.
...
#' @param plotLegend TRUE indicates a legend of the scorers should be printed on the plots, FALSE indicates this legend should not be printed
...
makereport_sensitivitybarplots <- function(data, species, plots.file.name,
                                            sensitivity=TRUE, plotDataQuality=FALSE,
                                            plotLegend=TRUE, preliminary=TRUE){

  # Function for plotting the stacked barcharts for all species
  ...
  #   plots.file.name     name to save the plots as
  ...
  #   plotDataQuality     TRUE indicacates the average data quality for each attribute
  #                       should be printed on the plot, FALSE indicates this should
  #                       not be printed on the plot
  #   plotLegend          TRUE indicates a legend of the scorers should be printed on the plots,
 ...
 # windows(w=8.5, h=11)
  ...
  pdf(paste(plots.file.name, '.pdf', sep=''), pointsize=12, width=8.5, height =11)
  ...
  for(i in 1:length(species)){ # for each species
    ...
    if(sensitivity==TRUE){
      rows <- which(data$Stock.Name==species[i] & !is.na(data$Scoring.Rank1) &
    ...
    for(j in 1:length(attributes)){ # draw a barchart on the page for each sensitivity attribute/exposure factors

      # get the data for this species and attribute
      rows <- which(data$Stock.Name==species[i] & data$Attribute.Name==attributes[j]
                    & !is.na(data$Scoring.Rank1))
      ...
      if(plotDataQuality==TRUE){
        title(main=paste(attributes[j], "\nData quality mean=", average.data.quality,sep=''),
              adj=0.05, font.main=1, cex.main=1, line=0.3)
      }else{
        title(main=attributes[j], adj=0.05, font.main=1, cex.main=1, line=0.2)
      }
    }
    ...
    # Add a legend of scorers along the top of plot if the plotLegend parameter equals TRUE
    if(plotLegend==TRUE){
      legend(-4, y.max*4.96, legend=species.scorers, fill=species.colors, horiz=T, bty='n',
             xpd=NA, cex=1.1, x.intersp = 0.05, xjust=0.5, yjust=0)
    }
