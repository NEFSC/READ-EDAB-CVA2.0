...
#' @param model.metrics a data.table containing the model metrics for each species. Generated by \code{make_performanceCSV}.
#'
#' @return Function does not return anything. Figures are saved to species-specific \code{figures} folder.


plot_SDMS <- function(species, type = c('ensemble', 'weights', 'importance', 'residuals'), yrStart, yrEnd, coastline, bathy, model.metrics){
    ...
    message(paste0("Plotting SDM Plots for ", s, "..."))
      ...
      if(ind[1]){
        ...
        #plot ensemble model results
          load(paste0(getwd(), '/', s, '/output_rasters/ENSEMBLE_', yrStart, '_', yrEnd, '.RData')) #abund
          ...
          pdf(paste0(file.path(getwd(),s, 'figures'), '/mean_SDM_', yrStart, '_', yrEnd, '.pdf'), width = 8, height = 11)
      ...
      if(ind[2]){
        ...
        #create 'clean' names
        model.metrics$Name <- gsub(' ', '', model.metrics$Common.Name)

        #make model weight barplots
          m <- as.matrix(model.metrics[which(model.metrics$Name == s), c(12:16)])
          ...
          aucs <- as.matrix(model.metrics[which(model.metrics$Name == s), c(7:11)])
      ...
      if(ind[4]){
        ...
        #residual plots
          ...
          #observations
          obs <- raster::stack(paste0(getwd(), '/', s, '/input_rasters/combined_rasters_', yrStart, '_', yrEnd, '.nc'))
          ...
          pdf(paste0(file.path(getwd(),s, 'figures'), '/mean_residuals_', yrStart, '_', yrEnd, '.pdf'), width = 8, height = 11)
          par(mfrow=c(4,3), mar = c(3,3,1,0))
          for(y in 1:12){
          ...
          pdf(paste0(file.path(getwd(),s, 'figures'), '/histogram_residuals_', yrStart, '_', yrEnd, '.pdf'), width = 6, height = 6)
