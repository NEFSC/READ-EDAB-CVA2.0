...
#' @param component_models character vector containing the component models to use for weight and importance plotting. 
...
make_sdm_plots <- function(
  ...
  for (s in species) {
      ...
      for (y in 1:12) {
        ...
        terra::plot(
          ...
          plg = list(
            ...
            x = -72,
            y = 36
          ), # Place inside bottom right of panel 12
        )
    ...
    if (ind[2]) {
      ...
      m <- as.matrix(model_metrics[
        which(model_metrics$Name == s),
        paste(toupper(component_models), "WT", sep = '.')
      ])
      grDevices::pdf(
        paste0(
          file.path(spp_dir, 'figures'),
      ...
      aucs <- as.matrix(model_metrics[
        which(model_metrics$Name == s),
        paste(toupper(component_models), "AUC", sep = '.')
      ])
      grDevices::pdf(
        paste0(
          file.path(spp_dir, 'figures'),
          '/component_model_aucs_',
          release,
          '.pdf'
        ),
        width = 6,
    ...
    if (ind[3]) {
      ...
      dfI <- rbind(
        rep(max(var_imp[-nrow(var_imp), ], na.rm = T), ncol(var_imp)),
        rep(0, ncol(var_imp)),
        var_imp[-nrow(var_imp), ]
      )
