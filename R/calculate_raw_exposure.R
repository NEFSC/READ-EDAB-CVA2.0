...
calculate_raw_exposure <- function(
  present,
  future,
  spatial_temporal,
  mask_bathy,
  ...
  if (spatial_temporal) {
    #if using spatially explicit present data
  ...
  #calculate exposure
  EXP <- normalize_model_data(
    raw = fAvg,
    avg = pAvg,
    sd = pSD,
    spatial_temporal = spatial_temporal
  ) #spatial_temporal matches what is used to calculate present avg/sd so that right methods are used
