...
#' @param auc.col name of the column containing the AUC values to be included in the table footnote
#' @param model.confidence a data.table containing the model confidence scores for each species.
#'
#' @return Function does not return anything. The table is saved to species- and timeframe-specific \code{figures} folder.

makereport_exposuretable <- function(species, presentTime, futureTime, variableDF, model.metrics, auc.col, model.confidence){
  ...
  for(s in species){
    ...
    #pull metrics
    #auc
    mod.auc <- model.metrics[which(model.metrics$Common.Name == s),auc.col]

    #model.confidence
    mc.mean <- model.confidence[which(model.confidence$Species == s), 'meanConfidence']
    mc.sd <- model.confidence[which(model.confidence$Species == s), 'sdConfidence']
    ####
    ...
    ####TIMESERIES
    #load timeseries
    load(paste0(file.path(getwd(),s, 'Data'), '/', presentTime, ' vs ', futureTime, '/variable_exposure_timeseries.RData')) #vecExp
    #subset timeseries matrix by rownames
    i <- rownames(vecExp) %in% names(cW)
    vecSub <- vecExp[i,]

    #load total exposure timeseries - all vars
    load(paste0(file.path(getwd(),s, 'Data'), '/', presentTime, ' vs ', futureTime, '/total_exposure_timeseries_all.RData')) #totalT
    totalAll <- totalT

    #load total exposure timeseries - important vars
    load(paste0(file.path(getwd(),s, 'Data'), '/', presentTime, ' vs ', futureTime, '/total_exposure_timeseries_subset.RData')) #totalT
    totalImp <- totalT
    ...
    for(x in 1:(nrow(timeTab)-2)){
      timeTab$Factor.Name[x] <- variableDF$Long.Name[which(variableDF$Short.Name == timeTab$Factor.Name[x])]
    }
    ...
    ##### MAPS
    #load maps
    load(paste0(file.path(getwd(),s, 'Data'), '/', presentTime, ' vs ', futureTime, '/variable_exposure_maps.RData')) #mapExp
    #subset timeseries matrix by rownames
    i <- names(mapExp) %in% names(cW)
    mapSub <- raster::subset(mapExp, which(i == T))

    #load total exposure maps - all vars
    load(paste0(file.path(getwd(),s, 'Data'), '/', presentTime, ' vs ', futureTime, '/total_exposure_maps_all.RData')) #totalM
    totalAll <- totalM

    #load total exposure maps - important vars
    load(paste0(file.path(getwd(),s, 'Data'), '/', presentTime, ' vs ', futureTime, '/total_exposure_maps_subset.RData')) #totalM
    totalImp <- totalM
    ...
    for(x in 1:(nrow(spaceTab)-2)){
      spaceTab$Factor.Name[x] <- variableDF$Long.Name[which(variableDF$Short.Name == spaceTab$Factor.Name[x])]
    }
    ...
    summary.table <- tabAll %>%
      ...
      text_transform(
        locations = cells_body(columns = dist_space),
        fn = function(x) {
          # Increase the height here to px(80) to fill the cell
    ...
    #Save
    gt::gtsave(summary.table, paste0(file.path(getwd(),s, 'Figures'), '/', presentTime, ' vs ', futureTime, '/exposure_summary_table.pdf'))
