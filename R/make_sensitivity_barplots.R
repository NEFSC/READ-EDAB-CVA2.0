...
make_sensitivity_barplots <- function(data, species, plots_name,
                                            sensitivity=TRUE, plot_data_quality=FALSE,
                                            plot_legend=TRUE, preliminary=TRUE, scorer = TRUE, scorer_dir){
 ...
 # windows(w=8.5, h=11)
  ...
  pdf(paste(plots_name, '.pdf', sep=''), pointsize=12, width=8.5, height =11)
  ...
  if(scorer){
    ...
    for(i in 1:num.scorers){ # got through each scorer to make plots for
      ...
      # save plots as pdf with specified name
      pdf(paste(scorer_dir, '/', scorers[i], '.pdf', sep=''), pointsize=12, width=8.5, height=11)
