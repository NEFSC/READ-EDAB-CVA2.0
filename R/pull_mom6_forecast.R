...
pull_mom6_forecast <- function(
  ...
  if (length(ind) > 1) {
    #if ind matches multiple files (which is the case for MLD because the names aren't unique)
  ...
  r_list_avg <- lapply(1:dim(varAvg)[3], function(i) {
    terra::rast(t(varAvg[,, i]))
  })
  rast_avg <- terra::rast(r_list_avg)

  terra::ext(rast_avg) <- c(
    min(lon[lonInd]),
    max(lon[lonInd]),
    min(lat[latInd]),
    max(lat[latInd])
  ...
  r_list_raw <- lapply(1:dim(var_flat)[3], function(i) {
    terra::rast(t(var_flat[,, i]))
  })
  rast_raw <- terra::rast(r_list_raw)

  terra::ext(rast_raw) <- c(
    min(lon[lonInd]),
    max(lon[lonInd]),
    min(lat[latInd]),
    max(lat[latInd])
  ...
  raw_names <- paste0(
    rep(time_names, times = 10),
    ".ens",
    rep(1:10, each = length(tm))
  )
