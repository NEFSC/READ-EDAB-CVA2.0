...
#' @description This is a wrapper function for \code{merge_spp_env}, \code{match_guilds}, \code{remove_corr}, and \code{clean_data} that produces logs and has skip functionality to assist in running multiple species in parallel.
...
makeDF <- function(name, skip, mMin, mMax, yMin, yMax){
  ...
  if(skip){
    if(file.exists(paste(file.path(getwd(),name), 'pa_clean.RData', sep = '/'))){
      ...
      combinedRasts <- raster::brick(paste(file.path(getwd(),name, 'input_rasters'), 'combined_rasters.nc', sep = '/')) #combinedRasts
      ...
      dfG <- match_guilds(spp_env = df, spp = name, spp_col = 'SCI_NAME', spp_guild = 'spp_list.csv', feeding_key = 'feeding_guilds.csv', feeding_col = 'Feeding.Guild', habitat_key = 'habitat_guilds.csv',  habitat_col = 'Habitat.Guild', static_vars = c('x', 'y', 'month', 'year', 'bathy', 'rugosity', 'dist2coast'), pa_col = 'value')
