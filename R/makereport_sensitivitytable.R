...
#'
#' @param species names of species to produce table for. Must match \code{Stock.Name} in \code{raw.data}
#' @param species_col name of column with species names to help subset data. Should be the same across \code{raw.data}, \code{sensitivity}, and \code{data.quality}
#' @param attribute_names_raw,attribute_names_clean vectors of raw attribute names to help subset \code{sensitivity} and \code{data.quality}, and cleaned up names to use in table
...
#' @param raw.data raw data exported from FCVA database, including the column \code{Stock.Name} to subset data to correct species
...
#' @param data.quality data.frame with data quality scores for each attribute and species. Generated by \code{data.quality.score}. Must include the column \code{Stock.Name} to subset data to correct species
...
#' @param table.folder file path to folder to save tables in
#'
#' @importFrom gt gt cols_label text_transform cells_body
#' @importFrom ggplot2 ggplot geom_col theme_void aes
...
makereport_sensitivitytable <- function(species, species_col, attribute_names_raw, attribute_names_clean, total_sens_col, certainty_col, raw.data, sensitivity, data.quality, table.folder){
  #raw.data is a data.frame of sensitivity data from the portal - used for tally counts/distribution plot
  #sensitivity - data.frame with attribute scores, overall sensitivity, and certainty
  #data.quality - data.frame of data quality scores for each attribute

  for(x in species){
    #subset each data.frame
    spRaw <- raw.data[raw.data[,species_col] == x,] #raw.data
    spSens <- sensitivity[sensitivity[,species_col] == x,] #sensitivity
    spDQ <- data.quality[data.quality[,species_col] == x,] #data.quality
    ...
    rownames(tab) <- NULL
    tab <- tab[,c(3,1,2)]
    colnames(tab)[2:3] <- c("expert.scores", 'data.quality')
    ...
    summary.table <- tab %>%
      ...
      gt::gt() %>%
      #title w/ overall sensitivity & confidence
      gt::tab_header(
        title = 'Sensitivity',
      ) %>%
      gt::tab_options(
      ...
      gt::cols_width(
        ...
        data.quality ~ gt::px(60),            # Forces all SD columns to 80px
      ...
      gt::tab_style(
        ...
        locations = gt::cells_body(columns = c('expert.scores', 'data.quality'))
      ) %>%
      gt::text_transform(
    ...
    gt::gtsave(summary.table, paste0(table.folder, '/sensitivity_table_', gsub(' ', '', x), '.pdf'))
} #end x
