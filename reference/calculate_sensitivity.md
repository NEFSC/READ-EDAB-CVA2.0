...
Wrapper function that calculate species sensitivity using
`attribute_score` and `apply_logic_rule` for individual attributes and
apply logic rule to calculate total sensitivity. The option to perform
bootstrapping is provided. Based on code from the Southeast Fisheries
...
- species_attributes:
  ...
  a list of data frames, with each data frame corresponding to all of
  the expert scores for one species. See example for how to generate
  this list from the FCVA output.
...
a data.frame where each row is a species, and columns represent
attribute scores, as well as the Total Sensitivity score column returned
by the logic rule
